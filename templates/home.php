        ...
        for (j = 1; j &lt;= gridj - 2; j++) {
            if (i == 1 || i == gridi - 2 || j == 1 || j == gridj - 2) {
                if (j &lt; gridi - l)
                    ...
                    omega[gridi - k - 1][j] = (v[gridi - k][j] - v[gridi - k - 1][j]) / dx;
                if (j &lt; l)
                    omega[k][j] = (v[k][j] - v[k - 1][j]) / dx;
                else
                    omega[gridi - 1][j] = (v[gridi - 1][j] - v[gridi - 2][j]) / dx;
                if (i &lt; k)
                    omega[i][0] = -(u[i][1] - u[i][0]) / dy;
                else
                    omega[i][l] = -(u[i][l + 1] - u[i][l]) / dy;
                if (i &lt; gridi - k)
                    omega[i][gridj - l - 1] = -(u[i][gridj - l - 1] - u[i][gridj - l - 2]) / dy;
                else
...
void Solve(uint is, uint ie, uint js, uint je)
{
    ...
    if (iteromega % 2 == 0) {
        ...
        for (j = js + 1; j &lt;= je - 1; j++) {
            ...
            d[is + 1] = d[is + 1] - a[is + 1] * omega[is][j];
            a[is + 1] = 0.0;
            d[ie - 1] = d[ie - 1] - c[ie - 1] * omega[ie][j];
            c[ie - 1] = 0.0;
        ...
        for (i = is + 1; i &lt;= ie - 1; i++) {
            ...
            d[js + 1] = d[js + 1] - a[js + 1] * omega[i][js];
            a[js + 1] = 0.0;
            d[je - 1] = d[je - 1] - c[je - 1] * omega[i][je];
            c[je - 1] = 0.0;
